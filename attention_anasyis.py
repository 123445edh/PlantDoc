...
def load_best_model(model_path='vit_lr_2e-05.pth', num_classes=27):
    """加载最佳模型"""
    ...
    if os.path.exists(model_path):
        ...
        print(f"模型权重加载成功! 验证准确率: 66.15%")
    else:
        print(f"警告: 模型文件 {model_path} 不存在")
        # 尝试其他模型
        alternative_models = ['vit_lr_5e-05.pth', 'ViT-Large_model.pth']
...
if __name__ == "__main__":
    # 加载最佳模型
    model = load_best_model('vit_lr_2e-05.pth', len(class_names))

    # 创建分析器
    analyzer = SimpleAttentionAnalyzer(model.model, class_names)
    ...
    plt.title('最佳模型混淆矩阵 (准确率: 66.15%)', fontsize=16)
